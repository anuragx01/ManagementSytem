...
class LeaveServiceTest {
    ...
    void setUp() {
        ...
        employeeId = UUID.fromString("00000000-0000-0000-0000-000000000002");
        leaveTypeId = UUID.fromString("00000000-0000-0000-0000-000000000003");
    ...
    void applyLeave_withSufficientBalance_savesRequest() {
        ...
        when(leaveTypeRepository.findByIdAndDeletedFalse(leaveTypeId)).thenReturn(Optional.of(leaveType));
        when(leaveRequestRepository.findOverlapping(any(), any(), any())).thenReturn(Collections.emptyList());
        when(leaveBalanceRepository.findByEmployeeIdAndLeaveTypeIdAndYearAndDeletedFalse(
                any(), any(), anyInt())).thenReturn(Optional.of(leaveBalance));
        ...
        request.setEndDate(LocalDate.of(2026, 7, 10));   // Friday
        request.setDayType(LeaveRequest.DayType.FULL);
        request.setReason("Annual vacation");

        // Act
        LeaveRequestResponse response = leaveService.applyLeave(request, currentUser);

        // Assert
        verify(leaveRequestRepository).save(any(LeaveRequest.class));
        verify(eventPublisher).publishEvent(any(LeaveNotificationEvent.class));
    ...
    void applyLeave_withInsufficientBalance_throwsException() {
        // Arrange
        LeaveBalance insufficientBalance = LeaveBalance.builder()
                .employee(employee)
                .leaveType(leaveType)
                .year(2026)
                .allocatedDays(2.0)
        ...
        when(leaveRequestRepository.findOverlapping(any(), any(), any())).thenReturn(Collections.emptyList());
        when(leaveBalanceRepository.findByEmployeeIdAndLeaveTypeIdAndYearAndDeletedFalse(
                any(), any(), anyInt())).thenReturn(Optional.of(insufficientBalance));

        // Request 5 working days but only 2 available
        ApplyLeaveRequest request = new ApplyLeaveRequest();
        request.setLeaveTypeId(leaveTypeId);
        request.setStartDate(LocalDate.of(2026, 7, 6));  // Monday
        request.setEndDate(LocalDate.of(2026, 7, 10));   // Friday (5 working days)
        request.setDayType(LeaveRequest.DayType.FULL);
        request.setReason("Vacation");

        // Act & Assert
        assertThatThrownBy(() -> leaveService.applyLeave(request, currentUser))
    ...
    void approveByManager_updatesStatusToManagerApproved() {
        // Arrange
        UUID managerId = UUID.fromString("00000000-0000-0000-0000-000000000010");
        UUID managerUserId = UUID.fromString("00000000-0000-0000-0000-000000000011");
        UUID leaveRequestId = UUID.fromString("00000000-0000-0000-0000-000000000020");

        UserPrincipal managerPrincipal = mock(UserPrincipal.class);
        when(managerPrincipal.getId()).thenReturn(managerUserId);
        ...
        when(employeeRepository.findByUserIdAndDeletedFalse(managerUserId)).thenReturn(Optional.of(manager));
        when(leaveRequestRepository.findByIdAndDeletedFalse(leaveRequestId)).thenReturn(Optional.of(pendingRequest));
        ...
        approvalRequest.setDecision(LeaveRequest.ApprovalDecision.APPROVED);
        approvalRequest.setRemarks("Approved");

        // Act
        LeaveRequestResponse response = leaveService.managerDecision(approvalRequest, managerPrincipal);
        ...
        assertThat(captor.getValue().getStatus()).isEqualTo(LeaveRequest.LeaveStatus.MANAGER_APPROVED);
    }

    @Test
    void calculateWorkingDays_excludesWeekends() {
        // Arrange - Monday July 6 to Sunday July 12 (7 calendar days, 5 working days)
        when(employeeRepository.findByUserIdAndDeletedFalse(userId)).thenReturn(Optional.of(employee));
        when(leaveTypeRepository.findByIdAndDeletedFalse(leaveTypeId)).thenReturn(Optional.of(leaveType));
        when(leaveRequestRepository.findOverlapping(any(), any(), any())).thenReturn(Collections.emptyList());
        when(leaveBalanceRepository.findByEmployeeIdAndLeaveTypeIdAndYearAndDeletedFalse(
        ...
        ArgumentCaptor<LeaveRequest> captor = ArgumentCaptor.forClass(LeaveRequest.class);
        when(leaveRequestRepository.save(captor.capture())).thenAnswer(inv -> inv.getArgument(0));
        when(leaveBalanceRepository.save(any(LeaveBalance.class))).thenAnswer(inv -> inv.getArgument(0));

        ApplyLeaveRequest request = new ApplyLeaveRequest();
        request.setLeaveTypeId(leaveTypeId);
        request.setStartDate(LocalDate.of(2026, 7, 6));   // Monday
        request.setEndDate(LocalDate.of(2026, 7, 12));    // Sunday
        request.setDayType(LeaveRequest.DayType.FULL);
        request.setReason("Testing weekend exclusion");

        // Act
        leaveService.applyLeave(request, currentUser);

        // Assert — Saturday + Sunday must be excluded, so totalDays = 5
        assertThat(captor.getValue().getTotalDays()).isEqualTo(5.0);
    }
}
